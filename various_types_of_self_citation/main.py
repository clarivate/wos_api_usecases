...
def cited_papers():  # This is how we create a list of cited papers based on a search query specified in the start
    ...
    for paper in data:  # Breaking the received JSON data into separate instances of cited_paper class
        ...
        cited_papers_list.append({
            'ut': ut, 'author_names': author_names, 'author_dais': author_dais, 'author_rids': author_rids,
            'author_orcids': author_orcids, 'org_names': org_names, 'country_names': country_names,
            'source_name': source_name, 'times_cited': times_cited, 'citing_papers_list': citing_papers_list
                                 })
...
def citing_papers(cited_papers_list):  # Based on the list of cited papers, we get a list of the records which cite them
    for paper in cited_papers_list:
        ...
        print(f"Now getting citing papers data for each of them: {cited_papers_list.index(paper) + 1} of "
              f"{len(cited_papers_list)}")
        ...
        for record in data:
            ...
            self_citation_crs = 0
            paper['citing_papers_list'].append({
                'ut': ut, 'author_names': author_names, 'author_dais': author_dais, 'author_rids': author_rids,
                'author_orcids': author_orcids, 'org_names': org_names, 'country_names': country_names,
                'source_name': source_name, 'self_citation_crs': self_citation_crs
                                 })
    return cited_papers_list
...
def citing_request(paper):  # This function  gets the citing paper data via API
    ...
    initial_data = initial_response.json()
    for i in range(((initial_data['QueryResult']['RecordsFound'] - 1) // 100) + 1):
        subsequent_response = requests.get(f"{baseurl}/citing?databaseId=WOS&uniqueId={paper['ut']}&count=100&"
...
def get_country_list(paper):
    ...
    try:
        if paper['static_data']['fullrecord_metadata']['addresses']['count'] == 1:
            country_names.add(
                paper['static_data']['fullrecord_metadata']['addresses']['address_name']['address_spec']['country']
            )
        else:
            for affiliation in paper['static_data']['fullrecord_metadata']['addresses']['address_name']:
...
def self_citation_crs_calc(cited_paper, citing_paper):
    ...
    for checked_citing_paper in checked_citing_papers:  # Checking if the paper has already been extracted via API
        ...
        initial_response = requests.get(f"{baseurl}/references?databaseId=WOS&uniqueId={citing_paper['ut']}&count=100&"
                                        f"firstRecord=1", headers=headers)
        cr_data = initial_response.json()
        for i in range(((cr_data['QueryResult']['RecordsFound'] - 1) // 100)):
            subsequent_response = requests.get(
                f"{baseurl}/references?databaseId=WOS&uniqueId={citing_paper['ut']}&count=100&firstRecord={i}01",
                headers=headers)
            addtl_cr_data = subsequent_response.json()
            for j in range(len(addtl_cr_data['Data'])):
                cr_data['Data'].append(addtl_cr_data['Data'][j])
    ...
    for cr in cr_data['Data']:  # Checking if the ID of a paper in cited reference matches the ID of a cited paper
        if cr['UID'] == cited_paper['ut']:
            citing_paper['self_citation_crs'] += 1  # If it does, the citing paper self-citation count is increased by 1
...
def self_citations(cited_papers_list):  # Self-citation calculations occur here
    ...
    for cited_paper in cited_papers_list:  # For every cited paper we run a check
        for citing_paper in cited_paper['citing_papers_list']:  # Every paper that was citing it is checked for matches
            if len(cited_paper['author_names'].intersection(citing_paper['author_names'])) > 0:  # For (co)author names
                # If at least 1 match is found, a calculation of references from citing to cited document is counted
                self_citation_crs_calc(cited_paper, citing_paper)
                print(f'Oops, seems like a self-citation found: paper {cited_papers_list.index(cited_paper) + 1} of '
                      f'{len(cited_papers_list)}')
                author_name_self_citation += citing_paper['self_citation_crs']
            if len(cited_paper['author_dais'].intersection(citing_paper['author_dais'])) > 0:  # For (co)author papers
            ...
            if len(cited_paper['country_names'].intersection(citing_paper['country_names'])) > 0:  # For their countries
                if citing_paper['self_citation_crs'] == 0:
                    self_citation_crs_calc(cited_paper, citing_paper)
                country_self_citation += citing_paper['self_citation_crs']
            if cited_paper['source_name'] == ['citing_paper.source_name']:  # For titles those papers were published in
                if citing_paper['self_citation_crs'] == 0:
                    self_citation_crs_calc(cited_paper, citing_paper)
    ...
    print(f'Coauthor self-citation:\n    Name-level: {(author_name_self_citation/total_citations * 100):.2f}% '
          f'({author_name_self_citation} self-citations, {total_citations - author_name_self_citation} external, '
          f'{total_citations} total)')
    print(f'    DAIS-level: {(author_dais_self_citation/total_citations * 100):.2f}% ({author_dais_self_citation} '
          f'self-citations, {total_citations - author_dais_self_citation} external, {total_citations} total)')
    print(f'    ResearcherID-level: {(author_rids_self_citation/total_citations * 100):.2f}% '
          f'({author_rids_self_citation} self-citations, {total_citations - author_rids_self_citation} external, '
          f'{total_citations} total)')
    print(f'    ORCID-level: {(author_orcids_self_citation/total_citations * 100):.2f}% ({author_orcids_self_citation} '
          f'self-citations, {total_citations - author_orcids_self_citation} external, {total_citations} total)')
    print(f'Organization-level self-citation: {(org_self_citation/total_citations * 100):.2f}% ({org_self_citation} '
          f'self-citations, {total_citations - org_self_citation} external, {total_citations} total)')
    print(f'Country-level self-citation: {(country_self_citation/total_citations * 100):.2f}% ({country_self_citation} '
          f'self-citations, {total_citations - country_self_citation} external, {total_citations} total)')
    print(f'Publication Source-level self-citation: {(source_self_citation/total_citations * 100):.2f}% '
          f'({source_self_citation} self-citations, {total_citations - source_self_citation} external, '
          f'{total_citations} total)')
